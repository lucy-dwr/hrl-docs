...
- Raw data: This can have multiple meanings but is used here to refer to data that has not been summarized.
- QC: Quality control refers to data cleaning and checks that are performed after data has been collected and stored. The are multiple sources of data issues anywhere from equipment failures to data entry typos. QC is performed to remove known data issues to the extent possible.
...
The data being published to EDI should not be summarized or reported in aggregate. Clean and QC'd data that have not been summarized or aggregated are most broadly useful. EDI functions as a public repository for your data, rather than a reporting portal. By publishing this "raw" or not summarized data, all data requests can be addressed through EDI.
